...
When /^I receive websocket notification for chat attachement "([^"]*)"$/ do |attachement|
  attachement_url = ChatAttachement.find_by_attachement_file_name(attachement).attachement.url
  page.execute_script('window.chat.handle_websocket_event($(".chat_window .chat_attachements").append("<div class=\'chat_attachement\'>  <a href=\"' + attachement_url + '\" target=\"_blank\">' + attachement + '<\/a><\/div>"));')
end
